	...
	if ($result) {
		if (mysqli_num_rows($result) == 0) {
			...
			while ($notification = mysqli_fetch_assoc($result)) {
				echo("\t\t\t<li><a href=\"http://{$notification['link']}\">{$notification['title']}</a> <span class=\"date\">" . date('d/m/y G:i', $notification['date']) . "</span><form action=\"javascript: formAction('deleteNotification.php', ['deleteNotif']);\" style=\"display: inline\"><input type=\"text\" id=\"deleteNotif\" value=\"{$notification['nid']}\" hidden=\"yes\"><input type=\"text\" name=\"viewNotifications\" value=\"{$viewNotificationsButtonTxt}\" hidden=\"yes\"><input type=\"submit\" value=\"Delete\"></form></li>\n");
			}
			echo("\t\t</ul>");
		}
	}
